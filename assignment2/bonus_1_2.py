...
#  more exhaustive random search, more hidden nodes
...
K, n, d, m=10, data.shape[1], data.shape[0], 200
...
def MiniBatchGD(X, y, n_batch, W1, b1, W2, b2, n_s, n_cycles, eta_min=1e-5, eta_max=1e-1):
    ...
    cost_lst=list()
    acc_lst=list()
    lamda=coarse_search_lamda(-5, -4, 3.38e-4)
    print(lamda, end=' ')
    n=X.shape[1]
