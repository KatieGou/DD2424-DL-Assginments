...
K, n, d, m=10, data.shape[1], data.shape[0], 50
...
def MiniBatchGD(X, y, n_batch, W1, b1, W2, b2, n_s, n_cycles, eta_min=1e-5, eta_max=1e-1):
    ...
    for i in range(int(n_epochs)):
        eta=cyclical_learning_rate(0.037, 1e-3, n_s, i*(n/n_batch))
...
def find_eta(base_lr=1e-3, max_lr=1e-1, stepsize=5e-3):
...
# final_acc_lst_validation=list()
# final_acc_lst_test=list()
# eta_array=find_eta()
...
# final_acc_lst_validation.append(max(acc_lst_validation))
# final_acc_lst_test.append(max(acc_lst_test))

# plt.plot(eta_array.tolist(), final_acc_lst_validation)
# plt.xlabel("eta")
# plt.ylabel("accuracy")
# plt.title("validation accuracy at different eta values")
# plt.grid()
# plt.show()
# print(max(final_acc_lst_validation))
# print(max(final_acc_lst_test))