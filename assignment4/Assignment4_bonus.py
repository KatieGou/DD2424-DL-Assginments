...
class Rewrite:
    def __init__(self, readfilename, writefilename):
        ...
        list_of_tweets = df['text'].to_list()
        all_tweets='\n'.join(list_of_tweets)
...
class Readfile:
    def __init__(self, filename, N):
        ...
        self.book_data=self.book_data[:N]
        self.book_chars=list(set(self.book_data))
        self.c2i={c: i for i, c in enumerate(self.book_chars)}
        self.i2c={i: c for c, i in self.c2i.items()}

class RNN:
    def __init__(self, book_data, c2i, i2c, m=100, seq_length=40, eta=0.1, mu=0, sig=0.01, e=0, epsilon=1e-8, epoch=8, smooth_loss=0, n=140):
    ...
    def train(self, plotting=True):
        ...
        for epoch in range(self.epoch):
            ...
            for iter in range(self.iter):
                ...
                if (iter+epoch*self.iter)%10000==0:
                    ...
                    print(s)
                    print()
                    with open('Text_Tweets_50.txt', 'a', encoding='utf8') as f:
        ...
        if plotting:
            plt.plot(loss_lst)
            plt.title('Smooth Loss Tweets')
            plt.xlabel('Iterations')
            plt.ylabel('Smooth Loss')
            plt.grid()
            plt.savefig('Loss_Tweets_50.png')
            plt.show()
...
def main():
    rewrite=Rewrite('gotTwitter.csv', 'gotTwitter.txt')
    readfile=Readfile('gotTwitter.txt', 1100001)
